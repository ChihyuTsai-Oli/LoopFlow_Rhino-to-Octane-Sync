...
本機封裝用，**不是** `v2.0.0` 上架包。版號 `0.1.1`。yak 含 Octane Lua（`templates/`）；第一次跑任一產品指令拷到「文件\LoopFlow\Rhino to OctaneRender Sync\lua」。不自動寫入 Octane Preferences。
...
裝完須**完全關 Rhino 再開**。不要跟 1.x 或開發按鈕混測。不上 Package Manager、不打 `v2.0.0`。
