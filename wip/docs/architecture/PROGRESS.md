...
- 狀態：重構模式已定案；**決策表多數已決**（含 ED-02＝A 手動置換、ED-05／10、ED-18＝JSON、ED-19＝B）；三家建議表已改名；操作模擬合併＋HTML 已產；開發順序先 R2B 再 R2O；命名契約待完整回寫；尚未修改產品程式碼

## AI 接手入口

本 repo 已建立自足的繁中維護文件。AI 開始前依序讀取根目錄 `AGENTS.md`、`wip/docs/_R2O_使用說明.md`、`wip/docs/_R2O_系統設定.md`、`wip/docs/_R2O_命名與資料契約.md`、`wip/docs/_R2O_重構計畫.md`、`wip/docs/architecture/DEVELOPMENT_ROADMAP.md`，最後讀本文件確認即時進度。若契約尚未定案，另讀 `wip/docs/前期規劃/資料生態決策表_三家建議.md`（尚待確認唯一來源）與 `wip/docs/rhino指令.md`（測試按鈕）。外部分析檔不再是必要輸入。
...
|---|---|---|---|---|
| 2026-08-27 | `codex/v2-decision-table` | 決策表改名 `_三家建議`；寫回 ED-02／05／10；合併模擬＋HTML；使用說明／命名契約已註記；開發序 R2B→R2O | 文件已更新；衍生 HTML／合併表已重產 | 尚未 commit |
| 2026-08-27 | `codex/v2-decision-table` | 三版操作模擬合併為 `操作流程模擬_合併.md`（衝突處加註） | 已產；後續已寫入 ED-02／05／10 | 尚未 commit |
| 2026-08-27 | `codex/v2-decision-table` | 補 `R2O-ED-19`：Octane Camera 再試可開關 real-time；更新模擬／使用說明／藍圖 | 說明已寫入；尚未寫產品碼 | 尚未 commit |
| 2026-08-27 | `codex/v2-decision-table` | `R2O-ED-18`＝B；合併建議表；操作模擬 Models→Camera→Point→Scatter | 衍生 md 已產 | 尚未 commit |
| 2026-08-27 | `codex/v2-decision-table` | 三欄 AI 多數決自動填「你的決定」；產生彩色 HTML；`XF-*` 自 R2B 同步 | 自動採用 38 列；**僅 `R2O-ED-18` 仍待決定**（Grok A vs Claude／Codex B） | HTML：白／黃／綠。尚未 commit |
...
1. 等 R2B 主鏈文件／契約較穩後，再完整回寫 R2O 命名契約／系統設定與 A01 盤點。
2. 日常覆核以 `操作流程模擬_合併.md`／`.html` 與 `資料生態決策表_三家建議.md` 為準；改口只改決策表。
