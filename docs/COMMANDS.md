...
Lua is **not** one copy per project. After a formal Rhino command, the default is `Documents\LoopFlow\Rhino to OctaneRender Sync\lua`. If that default folder is missing, the next command copies it back. You can put the whole pack anywhere as long as Octane’s Script directory points there. The hotkey table `R2O_Shortcuts.txt` lives next to the scripts. That folder also has `Set_Octane_Script_Directory.txt` (English then Chinese). After a yak upgrade, this lua folder is emptied and refilled with the official files, including the hotkey table.
